...
describe('formatWeek', () => {
  ...
  it('월의 마지막 주에 대해 올바른 주 정보를 반환한다', () => {
    expect(formatWeek(new Date('2024-11-26'))).toBe('2024년 11월 4주');
  });
...
describe('isDateInRange', () => {
  ...
  it('시작일이 종료일보다 늦은 경우 모든 날짜에 대해 false를 반환한다', () => {
    ...
    expect(isDateInRange(new Date('2024-07-31'), rangeEnd, rangeStart)).toBe(false);
    expect(isDateInRange(new Date('2024-08-01'), rangeStart, rangeEnd)).toBe(false);
  });
});
...
describe('fillZero', () => {
  ...
  test('value가 지정된 size보다 큰 자릿수를 가지면 원래 값을 그대로 반환한다', () => {
    expect(fillZero(1000000, 3)).toBe('1000000');
  });
  // 아래 테스트는 위의 테스트와 중복으로 보인다.
...
describe('formatDate', () => {
  it('날짜를 YYYY-MM-DD 형식으로 포맷팅한다', () => {
    expect(formatDate(new Date('2024-11-01'))).toBe('2024-11-01');
  });

  it('day 파라미터가 제공되면 해당 일자로 포맷팅한다', () => {
    expect(formatDate(new Date('2024-11-01'), 10)).toBe('2024-11-10');
  });

  it('월이 한 자리 수일 때 앞에 0을 붙여 포맷팅한다', () => {
    expect(formatDate(new Date('2024-01-22'))).toBe('2024-01-22');
  });
